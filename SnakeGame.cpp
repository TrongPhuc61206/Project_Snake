...
void newGameStart()
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                if (event.key.code == Keyboard::W && direction.y == 0)
                    direction = Vector2f(0, -snakeSize);
                else if (event.key.code == Keyboard::S && direction.y == 0)
                    direction = Vector2f(0, snakeSize);
                else if (event.key.code == Keyboard::A && direction.x == 0)
                    direction = Vector2f(-snakeSize, 0);
                else if (event.key.code == Keyboard::D && direction.x == 0)
                    direction = Vector2f(snakeSize, 0);
            }
